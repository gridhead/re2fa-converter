...
class Automata:
    ...
    def display(self):
        otptdata = ""
        otptdata = otptdata + "States: " + str(self.states) + "<br/>" \
                            + "Start state: " + str(self.startstate) + "<br/>" \
                            + "Final states: " + str(self.finalstates) + "<br/>" \
                            + "Transitions: " + "<pre>"
        for fromstate, tostates in self.transitions.items():
            for state in tostates:
                for char in tostates[state]:
                    otptdata = otptdata + "\t" + str(fromstate) + " -> " + str(state) + " on " + "'" + str(char) + "'"
            otptdata = otptdata + "\n"
    ...
    def getDotFile(self):
        dotFile = "digraph DFA {<br/>rankdir=LR<br/>"
        if len(self.states) != 0:
            dotFile += "root=s1<br/>start [shape=point]<br/>start->s%d<br/>" % self.startstate
            for state in self.states:
                if state in self.finalstates:
                    dotFile += "s%d [shape=doublecircle]<br/>" % state
                else:
                    dotFile += "s%d [shape=circle]<br/>" % state
            for fromstate, tostates in self.transitions.items():
                for state in tostates:
                    for char in tostates[state]:
                        dotFile += 's%d->s%d [label="%s"]<br/>' % (fromstate, state, char)
        dotFile += "}"
        return dotFile
